 ...
 */
//added after make file gave error for not being initialized 
blockHeader *heap_end = NULL;
//blockFooter makes it easier to call on main
...
//heap traverse for best-fit block
while (current->size_status != 1) {
int currentBlockSize = current->size_status & ~0x7;
bool isAllocated = current->size_status & 0x1;

        if (!isAllocated && currentBlockSize >= blockSize) {
            if (bestFitSize == -1 || currentBlockSize < bestFitSize) {
                bestFit = current;
...
//move to next block on heap
current = (blockHeader*)((char*)current + currentBlockSize);
    }
...
//splitting
   if (bestFitSize > blockSize) {
        int remainingSize = bestFitSize - blockSize;
//so if the block is larger , lets split it to see if its large enough to hold another block w the header
 if (remainingSize >= (int)(sizeof(blockHeader) + 8)) {
            newBlock = (blockHeader*)((char*)bestFit + blockSize);
            newBlock->size_status = remainingSize;
}
//need to check prev alloc 0x2 bit
 if (bestFit->size_status & 0x2) {
			if(newBlock){//does it exist
                newBlock->size_status |= 0x2; }
            }
			if(newBlock){
            blockFooter* footer = (blockFooter*)((char*)newBlock + remainingSize - sizeof(blockFooter));
            footer->size_status = remainingSize; }//update!!! footer
//now we can set best fit
            bestFit->size_status = blockSize | 0x1;

if (bestFit->size_status & 0x2) {
                bestFit->size_status |= 0x2; }// keeping the p-bit
}//exact fit, trying to update and keet the size
else{ bestFit->size_status |= 0x1; }

...
I would say that they are negatively correlated to one another because there seems to be a noticeable negative slope happening showing that as hp increases, mpg decreases. Even though at the end it starts to scatter a bit more, I don't think we should dismiss the fact that there could be a negative correlation happening. 
...
#TODO: code goes here.
observed_stat = cor(hp,mpg)
permutations = 10000
#vector that holds permutation corrs
permuted_corr = numeric(permutations)

#Permutation Test
set.seed(123)
for(i in 1:permutations){
  shuffle = sample(mpg)
  permuted_corr[i] = cor(hp, shuffle)
}

pvalue = sum(abs(permuted_corr) >= abs(observed_stat)) / permutations
pvalue
...
TODO: short discussion and visual guess of slope goes here.
